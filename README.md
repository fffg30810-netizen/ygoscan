...
1. `build_db.py` scarica UNA volta l'elenco carte da [YGOPRODeck](https://ygoprodeck.com/api-guide/)
   (14.559 carte, prezzi Cardmarket € e TCGplayer $, stampe con codice set e rarità) e le immagini
   piccole delle carte in `data/small/` (policy YGOPRODeck: scaricare e ri-ospitare, mai hotlink).
   Per ogni immagine calcola tre hash percettivi (pHash dell'illustrazione, pHash e dHash dell'intera
   carta) e scrive `docs/db.json` (~5 MB).
2. `docs/index.html` legge il db, apre la camera e ~10 volte al secondo ritaglia il riquadro guida,
   ...
   lo confronta con tutte le immagini del db (distanza di Hamming). Il miglior candidato viene
   accettato solo se: illustrazione ≤ 18 bit, carta intera (p+d) ≤ 40 bit, margine sul secondo
   candidato ≥ 4 bit, e la stessa carta vince su 2 frame consecutivi. Poi resta "bloccato" finché la
   carta non esce dal riquadro (3 frame senza match), così una carta non viene contata due volte.
...
- Carte con illustrazione gemella (es. Alien Mother / Alien Warrior) possono non essere accettate per
  il criterio del margine: meglio un rifiuto che un prezzo sbagliato.
